...
class ShortAutomationUI(AbstractComponentUI):
        ...
        with gr.Row(visible=False) as short_automation:
            with gr.Column():
                ...
                tts_engine = gr.Radio(
                    ...
                    value=AssetComponentsUtils.ELEVEN_TTS,
                    interactive=True,
                )
    ...
    def create_short(
        ...
        try:
            ...
            background_videos = (
                background_video_list * ((numShorts // len(background_video_list)) + 1)
            )[:numShorts]
            background_musics = (
                background_music_list * ((numShorts // len(background_music_list)) + 1)
            )[:numShorts]
            if tts_engine == AssetComponentsUtils.ELEVEN_TTS:
                ...
                voice_module = ElevenLabsVoiceModule(
                    ApiKeyManager.get_api_key("ELEVEN LABS"),
                    voice,
            ...
            for i in range(numShorts):
                ...
                yield self.embedHTML + "</div>", gr.Button.update(
                    visible=True
    ...
    def inspect_create_inputs(
        ...
        if not openai_key:
            raise gr.Error(
                "OPENAI API key is missing. Please go to the config tab and enter the API key."
            )
        ...
        if self.tts_engine == AssetComponentsUtils.ELEVEN_TTS and not eleven_labs_key:
            raise gr.Error(
                "ELEVEN LABS API key is missing. Please go to the config tab and enter the API key."
            )
